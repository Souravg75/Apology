/**
 * SORRY SISTER & SOUL BESTIE - INTERACTIVE EXPERIENCE JAVASCRIPT
 * 6-Page Emotional Journey with 10-Photo Moving 3D Carousel, Runaway 'No' Button Engine,
 * Web Audio Synthesizer, Real Memory Lightbox, Candle Blessing, and Personalization.
 */
...
const teasingQuotes = [
  ...
  "You love our bond too much to press No! 💕",
  ...
  "Come on, you know you want to forgive your best friend! 🥺"
];
...
// ==========================================================
// 3. PAGE NAVIGATION & ROUTING (6 STEPS)
// ==========================================================
function navigateToPage(pageNum) {
  if (pageNum < 1 || pageNum > 6) return;
  state.currentPage = pageNum;
  
  playChime(480 + pageNum * 40, 0.3);
  ...
  if (pageNum === 3) {
    ...
    launchMegaConfetti(100);
    playFanfare();
  } else if (pageNum === 6) {
    launchMegaConfetti(50);
...
function triggerRunaway(e) {
  ...
  let randomY = Math.max(padding + 60, Math.random() * maxHeight);
  ...
  if (forgiveSelect) {
    forgiveSelect.addEventListener('change', (e) => {
      if (e.target.value === 'no') {
        ...
        showToast("Nice try! But only 'YES' is accepted in this friendship! 😇");
...
function handleYesClick() {
  ...
  if (speechBubble) {
    speechBubble.textContent = "YAAAY! Best soul sister ever! ❤️✨ Redirecting...";
  }
  if (mascotFace) {
    mascotFace.textContent = "🥳";
  }

  showToast(`🎉 ${state.sisterName} officially pardoned you!`);
...
function claimCoupon(cardEl, couponName) {
  ...
  showToast(`🎉 '${couponName}' redeemed! Valid forever!`);
}
...
function restartExperience() {
  ...
  navigateToPage(1);
  showToast("🔄 Rewound back to the start with deep gratitude and care!");
}

// ==========================================================
// 7. PAGE 6: DYNAMIC 3D MOVING CAROUSEL & LIGHTBOX
// ==========================================================
...
function updateCarouselDisplay() {
  ...
  slides.forEach((slide, i) => {
    slide.classList.toggle('active', i === state.carouselIndex);
  });
  ...
  if (counter) {
    counter.textContent = state.carouselIndex + 1;
  }
}
...
function startCarouselAutoPlay() {
  ...
  state.carouselInterval = setInterval(() => {
    if (state.carouselAutoPlay && state.currentPage === 6) {
      nextSlide();
    }
  }, 3200); // Transitions smoothly every 3.2 seconds
}
...
function toggleCarouselAutoPlay() {
  state.carouselAutoPlay = !state.carouselAutoPlay;
  const playIcon = document.getElementById('play-icon');
  const playText = document.getElementById('play-text');

  if (state.carouselAutoPlay) {
    if (playIcon) playIcon.textContent = '⏸️';
    if (playText) playText.textContent = 'Auto-Moving';
    startCarouselAutoPlay();
    showToast('🎞️ Auto-slideshow running');
  } else {
    if (playIcon) playIcon.textContent = '▶️';
    if (playText) playText.textContent = 'Paused';
    stopCarouselAutoPlay();
    showToast('⏸️ Slideshow paused');
  }
}

function switchMemoryView(viewType) {
  const carouselView = document.getElementById('carousel-view');
  const gridView = document.getElementById('grid-view');
  const carouselTab = document.getElementById('tab-carousel-btn');
  const gridTab = document.getElementById('tab-grid-btn');
  ...
  if (viewType === 'carousel') {
    ...
    if (carouselTab) carouselTab.classList.add('active');
    if (gridTab) gridTab.classList.remove('active');
    startCarouselAutoPlay();
  } else {
    if (carouselView) carouselView.style.display = 'none';
    if (gridView) gridView.style.display = 'block';
    if (carouselTab) carouselTab.classList.remove('active');
...
function setupCarouselInteractions() {
  ...
  wrapper.addEventListener('mouseenter', () => {
    if (state.carouselAutoPlay) stopCarouselAutoPlay();
  });

  wrapper.addEventListener('mouseleave', () => {
    if (state.carouselAutoPlay) startCarouselAutoPlay();
  });
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    closeLightbox();
    closeCustomizer();
  } else if (e.key === 'ArrowRight' && state.currentPage === 6) {
    nextSlide();
  } else if (e.key === 'ArrowLeft' && state.currentPage === 6) {
    prevSlide();
  }
});

// Interactive Candle of Blessing
function lightCandleForHer() {
  const candleBox = document.getElementById('candle-blessing-box');
  const candleIcon = document.getElementById('candle-flame-icon');
  const candleBtn = document.getElementById('light-candle-btn');
  const candleTitle = document.getElementById('candle-title');
  const candleDesc = document.getElementById('candle-desc');

  if (!candleBox) return;

  state.candleLit = true;
  candleBox.classList.add('lit');

  if (candleIcon) candleIcon.textContent = '🕯️✨';
  if (candleTitle) candleTitle.textContent = `Candle Lit for ${state.sisterName} 🌟`;
  if (candleDesc) candleDesc.textContent = "May your path be forever blessed with boundless happiness, radiant health, and peaceful smiles. Amen! 🕊️";

  if (candleBtn) {
    candleBtn.innerHTML = '<span>🕊️ Candle Lit & Prayer Sent Forever</span>';
    candleBtn.style.background = '#28a745';
  }

  playChime(523.25, 0.8);
  setTimeout(() => playChime(659.25, 0.8), 200);
  setTimeout(() => playChime(783.99, 1.0), 400);

  const rect = candleBox.getBoundingClientRect();
  for (let i = 0; i < 20; i++) {
    spawnMiniHearts(rect.left + rect.width / 2, rect.top + rect.height / 2);
  }

  showToast(`🕊️ A silent prayer for ${state.sisterName}'s happiness has been sent!`);
}

// ==========================================================
// 8. CUSTOMIZATION & LOCAL STORAGE
// ==========================================================
...
// ==========================================================
// 10. BACKGROUND FLOATING HEARTS & PARTICLES
// ==========================================================
...
function initBgHearts() {
  ...
  bgHearts = [];
  const heartCount = Math.floor(window.innerWidth / 40);
  for (let i = 0; i < heartCount; i++) {
    bgHearts.push({
      ...
      color: ['#ff85a2', '#fbb1bd', '#b388ff', '#ffb4a2', '#fbb034'][Math.floor(Math.random() * 5)]
    });
  }
...
function launchMegaConfetti(count = 100) {
  ...
  const colors = ['#ff4071', '#9b5de5', '#f15bb5', '#fee440', '#00f5d4', '#ff70a6', '#fbb034'];
